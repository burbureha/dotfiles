import { ELEMENT_NODE, TEXT_NODE, BLOCK_ELEMENTS, ATTRIBUTE_NODE, COMMENT_NODE, NODE_END, SVG_NAMESPACE } from '../shared/constants.js';
...
import { localCase, knownAdjacent, ignoreCase } from '../shared/utils.js';
...
const attributesHandler = {
  get(target, key) {
    return key in target ? target[key] : target.find(({
      name
    }) => name === key);
  }
...
class Element extends ParentNode {
  ...
  setAttribute(name, value) {
    if (name === 'class') this.className = value;else {
      const attribute = this.getAttributeNode(name);
      if (attribute) attribute.value = value;else setAttribute(this, new Attr(this.ownerDocument, name, value));
    }
  }
  ...
  attachShadow(init) {
    if (shadowRoots.has(this)) throw new Error('operation not supported'); // TODO: shadowRoot should be likely a specialized class that extends DocumentFragment
        ...
        case NODE_END:
          {
            ...
            if (isOpened) {
              if ('ownerSVGElement' in start) out.push(' />');else if (isVoid(start)) out.push(ignoreCase(start) ? '>' : ' />');else out.push(`></${start.localName}>`);
              isOpened = false;
